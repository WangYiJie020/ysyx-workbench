...
void sim_dump_statistics() {
  ...
  size_t totByType = idu_counter.totalInstCountSumByType();
	fmt::println("  by type: (total {})", totByType);
  for (size_t i = 0; i < IDUPerfCounter::TYPE_NUM; i++) {
    auto type_name =
        IDUPerfCounter::name_of_type((IDUPerfCounter::InstType)i);
    auto type_count = idu_counter.type_count[i];
    auto type_percentage =
        totByType == 0 ? NAN : ((double)type_count / (double)totByType) * 100.0;
		fmt::println(
        "    {:<10} : {:>6} ({:>5.2f}%) cpi {:.2f}", type_name, type_count,
        type_percentage,
        idu_counter.averageCPIOfType((IDUPerfCounter::InstType)i));
  }
  size_t totByFmt = idu_counter.totalInstCountSumByFmt();
	fmt::println("  by fmt: (total {})", totByFmt);
  for (size_t i = 0; i < IDUPerfCounter::FMT_NUM; i++) {
    auto fmt_name = IDUPerfCounter::name_of_fmt((IDUPerfCounter::InstFmt)i);
    auto fmt_count = idu_counter.fmt_count[i];
    auto fmt_percentage =
        totByFmt == 0 ? NAN : ((double)fmt_count / (double)totByFmt) * 100.0;
		fmt::println(
        "    {:<10} : {:>6} ({:>5.2f}%) cpi {:.2f}", fmt_name, fmt_count,
        fmt_percentage,
        idu_counter.averageCPIOfFmt((IDUPerfCounter::InstFmt)i));
  }
